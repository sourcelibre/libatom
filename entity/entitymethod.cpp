...
bool EntityMethod::call(const atom::Message &arguments, atom::Message & return_value)
{
    if (this->getArgumentsType() != "*")
    {
        std::string tags = atom::getTypeTags(arguments);
        if (tags != this->getArgumentsType())
        {
            std::ostringstream os;
            os << "Bad arg types " << tags << " for method " << this->getName() <<

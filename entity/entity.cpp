...
const atom::Message &Entity::getAttributeValue(const char *name) const
    throw(BadIndexException)
{
...
void Entity::setAttributeValue(const char *name, const atom::Message &value)
    throw(BadIndexException, BadAtomTypeException)
{
    ...
    {
        if (atom::getTypeTags(current->getValue()).compare(getTypeTags(value)) == 0)
        {
            ok_to_change = true;
        }
        else
        {
            ...
            // else
            std::ostringstream os;
            os << "Entity::" << __FUNCTION__ << ": Attribute " << name << ": Bad type " << atom::getTypeTags(value) << " while expecting " << atom::getTypeTags(current->getValue());
    ...
    if (ok_to_change)
    {
        ...
        ok_to_change = this->onAttributeChanged(name, value);
        if (ok_to_change)
...
bool Entity::callMethod(const char *method_name, const atom::Message &arguments, atom::Message & return_value)
    throw(BadIndexException)
{
